...
export default function QuotePage() {
  ...
  const handleFiles = async (newFiles: File[]) => {
    ...
    if (result) {
      setFiles(prev => prev.map(f => f.id === uploadedFiles[0].id ? { ...f, status: "success" } : f));
    } else {
      setFiles(prev => prev.map(f => f.id === uploadedFiles[0].id ? { ...f, status: "error" } : f));
    }
  };
  ...
  const removeFile = (id: string) => {
    setFiles((prev) => {
      ...
      if (filtered.length === 0) resetGeometry();
      return filtered;
    });
  };
                  ...
                  {isAnalyzing ? (
                    ...
                    <div className="space-y-4 mb-6">
                      <div className="p-4 rounded-lg bg-[var(--accent)]/5 border border-[var(--accent)]/20">
                        <p className="text-xs font-mono text-[var(--accent)] mb-1 uppercase tracking-wider">Estimated Price</p>
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs font-mono">
                        <div className="p-2 border border-[var(--border)] rounded bg-[var(--bg-primary)]">
                          <p className="text-[var(--text-muted)] mb-1">Volume</p>
                          <p>{(analysis.volume / 1000).toFixed(2)} cm³</p>
                        </div>
                        <div className="p-2 border border-[var(--border)] rounded bg-[var(--bg-primary)]">
                          <p className="text-[var(--text-muted)] mb-1">Dimensions</p>
                          <p>{formatDimension(analysis.boundingBox.dimensions[0])} × {formatDimension(analysis.boundingBox.dimensions[1])}</p>
                        </div>
                        <div className="p-2 border border-[var(--border)] rounded bg-[var(--bg-primary)]">
                          <p className="text-[var(--text-muted)] mb-1">Material</p>
                          <p>{analysis.estimatedMaterial.toFixed(1)}g PLA</p>
                        </div>
                        <div className="p-2 border border-[var(--border)] rounded bg-[var(--bg-primary)]">
                          <p className="text-[var(--text-muted)] mb-1">Print Time</p>
                          <p>{analysis.estimatedPrintTime.toFixed(1)} hrs</p>
                        </div>
                      </div>
                    </div>
                  ) : null}
